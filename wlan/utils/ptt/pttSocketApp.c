...
#else
#define AID_VENDOR_QCOM_DIAG 2950
#endif
 ...
 */
const gid_t groups[] = {AID_INET, AID_NET_ADMIN, AID_VENDOR_QCOM_DIAG, AID_WIFI};
...
void pttSocketAppProcNetlinkMsg (void *arg)
{
    ...
    do
    {
        ...
        LOG_PSA_V("NL MSG, len[%03d], NL type[0x%X] WNI type[0x%4hX] len[%03d]",
                  wnl->nlh.nlmsg_len, wnl->nlh.nlmsg_type, ntohs(wnl->wmsg.type), ntohs(wnl->wmsg.length));
      ...
      // messages to the client (Perl script)
        ...
        if (pserver->diag_msg.diag_msg_received == TRUE || ntohs(wnl->wmsg.type) == PTT_FTM_CMDS_TYPE)
        {
            ...
            if(0 >= contentsLength)
            {
               LOG_PSA_E("Invalid Contents Length %d", contentsLength);
               break;
            }
...
int pttSocketAppCapHandle(void)
{
   ...
   if (setgroups(/*sizeof(groups)/sizeof(groups[0])*/ 4, groups) != 0)
   {
...
PACK(void *) wlan_ftm_func_75(PACK(void *)req_pkt, uint16 pkt_len)
{
    ...
    if ((1 == ftmCommandType) && (!is_ffbm_mode))
       {
        /* insmod wlan.ko driver */
        LOG_PSA_E("NOT FFBM mode, cannot start FTM driver");
        free(pBuf);
        return NULL;
    }
